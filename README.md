...
df_main (результат объединения данных, полученных путём парсинга с сайта Московской биржи);
df_info (аггрегированная по дате и коду инструмента df_main),;
df_scaled (отскалированная с помощью MinMaxScaler таблица df_info);
df_r (таблица доходностей). 

В ходе анализа были найдены значения корреляции таргетированного показателя с остальными инструментами, построены графики рассеивания. Был получен вывод о том, что Сбербанк и Сургутнефтегаз имеют высокую положительную связь по графикам, но связь между ценой на доллар и ценой на акции сомнительна, хотя есть теоретические показатели для роста Сургута.

Также были построены графики динамики цен и гистограммы распределения для каждого из инструментов, была найдена таблица их доходностей. 
Далее мы нашли средние значения доходностей по показателям, стандартные отклонения, VAR и ES, построили график зависимости последних (с данным графиком можно ознакомиться в html версии). Также была найдена корреляция доходности SNGS с остальными инструментами.

В конце были построены графики динамики акций Сургута и скользящей средней при различных window size. Был получен вывод о том, что динамика акций Сургута в целом разочаровывающая, с плоской тенденцией и сезонными колебаниями из-за выплаты дивидендов летом. После краха акции медленно растут, но не приводят к общей положительной тенденции. На последнем графике заметен значительный шум.
